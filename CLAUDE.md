...
**Mission**: Build a complete, production-ready skill package for Project Fluent (Mozilla i18n) and publish it under the OpenAEC Foundation on GitHub. This is your standing order for every session in this workspace.
...
**End state**: A published GitHub repo at `https://github.com/OpenAEC-Foundation/Fluent-i18n-Claude-Skill-Package` with:
...
- Repository topics set (claude, skills, fluent, i18n, localization, ai, deterministic, openaec)
...
## Project Identity
- Project Fluent (Mozilla i18n) skill package for Claude — single technology, full-stack localization
- Technology: Project Fluent — Mozilla's localization system with natural-sounding translations using FTL syntax
- Languages: FTL (Fluent Translation List), TypeScript
...
| docs/masterplan/fluent-masterplan.md | Planning | Execution plan with phases, prompts, dependencies |
| README.md | Public | GitHub landing page |

## Technology Scope
| Tech | Prefix | Versions |
|------|--------|----------|
| Project Fluent | fluent- | @fluent/bundle 0.18+, @fluent/react 0.15+, @fluent/langneg |

## Skill Categories
| Category | Purpose | Naming |
|----------|---------|--------|
| syntax/ | FTL syntax, message patterns, selectors, terms | fluent-syntax-{topic} |
| impl/ | Integration workflows, React setup, locale management | fluent-impl-{topic} |
| errors/ | FTL parsing errors, resolution failures, fallback issues | fluent-errors-{topic} |
| core/ | Architecture, bundle API, negotiation, concepts | fluent-core-{topic} |
| agents/ | Intelligent orchestration | fluent-{agent-name} |
...
├── docs/
│   ├── masterplan/              # fluent-masterplan.md
│   └── research/                # vooronderzoek-fluent.md, topic-research/, fragments/
└── skills/
    └── source/
        ├── fluent-syntax/       # FTL syntax, message patterns, selectors, terms, attributes
        ├── fluent-impl/         # React integration, locale loading, bundle setup
        ├── fluent-errors/       # Parse errors, resolution failures, debugging
        ├── fluent-core/         # API overview, architecture, negotiation
        └── fluent-agents/       # Validation, FTL generation agents
```
Single technology package — all skills share the `fluent-` prefix.
...
5. **Read docs/masterplan/fluent-masterplan.md** → Know the execution plan and current phase details
...
- Relevant constraints from **DECISIONS.md** (D-003: English-only, D-007: FTL+TS dual coverage, D-008: 500 line limit, etc.)
...
- FTL syntax coverage with correct Fluent patterns
- Dual-language coverage (FTL + TypeScript for integration skills)
...
6. FTL + TypeScript dual coverage for integration skills
...
### Before ANY research:
1. Read **SOURCES.md** → Know approved sources for Project Fluent
2. Read **REQUIREMENTS.md** → Know what the research must cover
3. Read **DECISIONS.md** → Know constraints (D-003 English-only, D-006 FTL spec only, D-009 WebFetch, etc.)

### During research:
- Use ONLY sources listed in **SOURCES.md** (or add new ones there)
- Verify FTL examples against official Fluent specification
- Identify anti-patterns from real GitHub issues
- Use WebFetch to ensure latest documentation is consulted (D-009)
...
- Vooronderzoek: `docs/research/vooronderzoek-fluent.md`
...
- SKILL.md < 500 lines (per **DECISIONS.md** D-008), heavy content in references/
- YAML frontmatter: name + description with trigger words
- Structure: Quick Reference > Decision Trees > Patterns > Reference Links
- Naming: `fluent-{category}-{topic}`
- Dual coverage: Integration skills MUST show both FTL and TypeScript (per D-007)
...
### Repository: OpenAEC-Foundation/Fluent-i18n-Claude-Skill-Package

### Pre-Publication Checklist:
1. All skills validated (Phase 6 complete)
2. INDEX.md created with full skill catalog
3. README.md updated with final skill count and installation instructions
4. CHANGELOG.md has release entry
5. LICENSE file present (MIT)
6. .gitignore excludes local files

### Publication Steps:
1. Final commit with all changes
2. Create GitHub release tag (v1.0.0)
3. Push to OpenAEC-Foundation organization
4. Verify README renders correctly on GitHub
5. Submit to agentskills.org registry

### Post-Publication:
- Monitor GitHub issues for community feedback
- Periodic updates when Fluent packages release new versions
- Cross-link from other OpenAEC skill packages
